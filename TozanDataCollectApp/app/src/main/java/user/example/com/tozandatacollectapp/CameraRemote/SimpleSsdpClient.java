...
public class SimpleSsdpClient {
    ...
    public synchronized boolean search(final SearchResultHandler handler) {
        if (mSearching) {
            System.out.println(TAG + "search() already searching.");
            return false;
        }
        if (handler == null) {
            throw new NullPointerException("handler is null.");
        }
        System.out.println(TAG + "search() Start.");
        ...
        new Thread() {
            ...
            public void run() {
                ...
                try {
                    ...
                    // send 3 times
                    System.out.println(TAG + "search() Send Datagram packet 3 times.");
                    socket.send(packet);
                    Thread.sleep(100);
                    socket.send(packet);
                    Thread.sleep(100);
                    ...
                    System.out.println(TAG + "search() DatagramSocket error:" + e);
                    ...
                    System.out.println(TAG + "search() IOException :" + e);
                    if (socket != null && !socket.isClosed()) {
                        socket.close();
                    }
                    handler.onErrorFinished();
                ...
                try {
                    ...
                    while (mSearching) {
                        ...
                        if (!foundDevices.contains(ddUsn)) {
                            ...
                            if(ddUsn != null && !ddUsn.isEmpty()){
                                handler.onDeviceFound(ddLocation);
                            }
                        }
