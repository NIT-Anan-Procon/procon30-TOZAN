...
int dist_min=15;      //距離の最小値保存変数15で初期化
...
// 距離の計算　
  ...
  if(distance>dist_max && distance<50) dist_max=distance; //max値を更新
  if(distance>50) dist_max=28; //距離が異常値になった場合max値を28に再設定
  

/* 距離キー設定
*  (dist_max-dist_min)/10の1,2,2,2,2,1幅を0～5のステップに割り当て
*/
  int distance_step;//距離の6段階保存変数
  for(distance_step=0;distance_step<=5;distance_step++){
    if(distance < (distance_step*2+1)*(dist_max-dist_min)/10+dist_min)break;
  }
